...
const FinalSeatMapWithDynamicSections = ({ event, selectionDate }) => {
      ...
      {seats.map((seat) => {
        ...
        return (
          <button
            ...
            className={`text-white border border-black rounded-tr-full text-[10px] flex items-center justify-center gap-1 px-1 py-1 ${isReserved ? "cursor-not-allowed" : "cursor-pointer hover:opacity-80"}`}
          >
            <MdEventSeat /> {seat}
          </button>
        );
  ...
  return (
    ...
    >
      ...
      {/* Stage */}
      <div
        className="bg-cover bg-center w-[30%] min-h-[100px] flex mb-4 justify-center items-center rounded-[10px] shadow-[0px_4px_10px_rgba(0,0,0,0.1)]"
        style={{
      ...
      <div className="w-full max-w-3xl flex flex-col items-center mt-6 gap-4">
        ...
        <button
          ...
          className="mt-[10px] text-[16px] font-bold py-[10px] px-[80px] bg-green-600 hover:bg-green-800 text-white rounded transition-colors"
        >
          Confirm booking
        </button>
      </div>
